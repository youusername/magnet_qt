...
void Downloader::installUpdate()
{
    ...
    m_ui->downloadLabel->setText (tr ("下载完毕!"));
    m_ui->timeLabel->setText     (tr ("安装程序将单独打开")
                                  + "...");
    ...
    box.setInformativeText (tr ("点击 \"OK\" 开始安装更新"));

    QString text = tr ("要安装更新，您可能需要 "
                      "退出应用.");

    if(m_mandatoryUpdate)
        text = tr ("要安装更新，您可能需要 "
                   "退出申请。 这是强制更新，现在退出将关闭应用程序");
    ...
    /* Wait */
        ...
        m_ui->timeLabel->setText (tr ("点击 \"Open\" 按钮 "
                                      "应用更新"));
    }
}
...
void Downloader::cancelDownload()
{
    if (!m_reply->isFinished()) {
        QMessageBox box;
        box.setWindowTitle (tr ("更新"));
        box.setIcon (QMessageBox::Question);
        box.setStandardButtons (QMessageBox::Yes | QMessageBox::No);

        QString text = tr("您确定要取消下载吗?");
        if (m_mandatoryUpdate)
        {
            text = tr("您确定要取消下载吗？ 这是强制更新，现在退出将关闭应用程序");
        }
        box.setText (text);
